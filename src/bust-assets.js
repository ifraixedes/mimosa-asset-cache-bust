...
module.exports = function (mimosaConfig) {
  var moduleCache = {};
  var logger = mimosaConfig.log;

  var bustAllAssets = function bustAllAssets(mimosaConfig, next) {
  ...
  var renameAssets = function renameAssets(assetsFileList, digesterFunc, splitter, callback) {
  ...
  var cleanBustAssets = function cleanBustAssets(mimosaConfig, next) {
    var thisModuleConfig = mimosaConfig[mimosaConfigId];
    var next = next || function () {};
          ...
          fs.readFile(bustedPathFileName, function (err, fileContent) {
            ...
            digester(fileContent, function (err, signature) {
              ...
              fs.unlink(bustedPathFileName, function (err) {
                if (err) {
                  logger.error(moduleName + ' failed when deleting the busted file: ' + bustedPathFileName + ', probably due other process deleted it before this process. Details: ' + err.message);
                } else {
